...
Function MouseLook()
	...
	If KillTimer >= 0 And FallTimer >=0 Then
		...
		;k?¡è?¡ènnet?¡è?¡èn kameraa sivulle jos pelaaja on vammautunut
							...
							Else
								For i = 2 To 6
									If KeyHit(i) Then
										...
										;jos nykyist?¡è kanavaa ollaan soitettu, laitetaan jatketaan toistoa samasta kohdasta
...
Function UpdateMTF%()
	...
	;mtf ei viel?¡è spawnannut, spawnataan jos pelaaja menee tarpeeksi l?¡èhelle gate b:t?¡è
	If MTFtimer = 0 Then
...
Function CircleToLineSegIsect% (cx#, cy#, r#, l1x#, l1y#, l2x#, l2y#)
	...
	;Palauttaa:
	;  True (1) kun:
	;      Ympyr?¡è [keskipiste = (cx, cy): s?¡ède = r]
	;      leikkaa janan, joka kulkee pisteiden (l1x, l1y) & (l2x, l2y) kaitta
	;  False (0) muulloin
	
	;Ympyr?¡èn keskipisteen ja (ainakin toisen) janan p?¡è?¡ètepisteen et?¡èisyys < r
	;-> leikkaus
	...
	;Vektorit (janan vektori ja vektorit janan p?¡è?¡ètepisteist?¡è ympyr?¡èn keskipisteeseen)
	...
	;Janan p?¡è?¡ètepisteiden kautta kulkevan suoran ;yht?¡èl??; (ax + by + c = 0)
	Local a# = (l2y - l1y) / (l2x - l1x)
	Local b# = -1
	Local c# = -(l2y - l1y) / (l2x - l1x) * l1x + l1y
	
	;Ympyr?¡èn keskipisteen et?¡èisyys suorasta
	Local d# = Abs(a * cx + b * cy + c) / Sqr(a * a + b * b)
	
	;Ympyr?¡è on liian kaukana
	;-> ei leikkausta
	If d > r Then Return False
	
	;Local kateetin_pituus# = Cos(angle) * hyp
	
	;Jos p?¡è?¡èst?¡è?¡èn t?¡ènne saakka, ympyr?¡è ja jana leikkaavat (tai ovat sis?¡èkk?¡èin)
	Return True
End Function
